...
module ibex_core #(
    ...
    // Spm Interface
    output logic [31:0] spm_addr_o,
    output logic        mem_spm_o,
    output logic        spm_reg_o,
    output logic        del_spm_o,
    output logic        off_base_o,
    output logic        data_base_o,
    output logic        spm2_reg_o,
       
    input logic  [31:0] spm_data_i,        
    input logic         spm_hit_i,
    input logic         spm_gnt_i,
    input logic         spm_miss_i,     
...
`endif
  ...
  assign spm_read_ex = spm_reg_lsu || spm2_reg_lsu;
  
  logic [31:0]  regfile_wdata_spm;
  logic         spm_data_valid;
  logic [31:0]  spm_data;
  
  spm_lsu core_spm_unit (
      .clk_i            ( clk           ),
      .rst_ni           ( rst_ni        ),
      ...
      .spm_reg          ( spm_reg_lsu    ),
      .del_spm          ( del_spm_lsu    ),
      .off_base         ( off_base_lsu ),
      .spm2_reg         ( spm2_reg_lsu ),         
      
      .spm_data_valid_o ( spm_data_valid ),
      .spm_data_miss_o  ( ),
      .spm_data_o       ( spm_data ),
           
      .alu_mem_addr     ( alu_adder_result_ex ),    
    
      .spm_data_i       ( spm_data_i ),     
      .spm_data_hit_i   ( spm_hit_i ), 
      .spm_data_gnt_i   ( spm_gnt_i ),   
      .spm_data_miss_i  ( spm_miss_i),    
      
      .spm_addr_o       ( spm_rd_addr_core ),       
      .mem_spm_valid    ( mem_spm_top     ),
      .spm_reg_valid    ( spm_reg_top     ),
      .del_spm_valid    ( del_spm_top     ),
      
      .off_base_valid   ( off_base_top ),
      .data_base_valid  ( data_base_top ),
      
      
      .spm2_reg_valid   ( spm2_reg_top )   
  );
  
  assign spm_addr_o         =  spm_rd_addr_core;
  assign mem_spm_o          =  mem_spm_top;
  assign spm_reg_o          =  spm_reg_top;
  assign del_spm_o          =  del_spm_top;
  assign off_base_o         =  off_base_top;
  assign data_base_o        =  data_base_top;
  assign spm2_reg_o         =  spm2_reg_top;  
  assign regfile_wdata_spm  = spm_data;
  ...
  ibex_id_stage #(
      ...
      //spm_lsu
      .spm_data_valid_i             ( spm_data_valid ),
      .regfile_wdata_spm_i          ( regfile_wdata_spm      ),      
      .spm_lsu_o                    ( spm_lsu_en             ),
      .mem_spm_id_o                 ( mem_spm_lsu            ),
      .spm_reg_id_o                 ( spm_reg_lsu            ),      
      .del_spm_id_o                 ( del_spm_lsu            ),
      .off_base_id_o                ( off_base_lsu ),
      .spm2_reg_id_o                ( spm2_reg_lsu ),         
...
`endif
  ...
  ibex_ex_block #(
      ...
      .spm_read_en_i              ( spm_read_ex           ),
      
      .spm_read_ready_i           ( spm_data_valid ),  
